    ...
    return(
        <section className="w-full max-w-full flex-start flex-col">
            ...
            className="mt-10 w-full max-w-2xl flex flex-col gap-7 glassmorphism">
                ...
                <textarea 
                value={post.prompt} onChange={(event)=>setPost({...post,prompt:event.target.value})}
                placeholder="Write your prompt here..." required className="form_textarea"/>
                </label>
                <label>
                    <span className="font-statoshi font-semibold text-base text-gray-700">
                        Tag {' '}
                        <span className="font-normal">(#product, #webdevelopement, #idea)</span>
                    </span>
                    <input value={post.tag} onChange={(e)=>setPost({...post, tag:
                        e.target.value})} placeholder="$tag" required className="form_input" />
                </label>
